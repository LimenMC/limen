...
impl NeoForgeInstaller {
    ...
    pub async fn install_neoforge(
        ...
        let installer_path = version_dir.join("neoforge-installer.jar");
        
        self.download_installer(&installer_url, &installer_path).await?;
        let (version_json, install_profile, libs_to_download) = 
            self.extract_data(&installer_path, game_version, &neoforge_version_name)?;
        
        self.save_version_json(&version_dir, &neoforge_version_name, &version_json)?;
        ...
        if let Some(profile) = &install_profile {
            let _ = self.run_neoforge_processors(&installer_path, profile, game_version);
        }
    ...
    fn cleanup(&self, installer_path: &Path) {
        ...
        let _ = fs::remove_dir_all(self.limen_dir.join("minecraft").join("neoforge_data_temp"));
    }
    ...
    fn extract_data(
        ...
        installer_path: &Path,
        game_version: &str,
        neoforge_version_name: &str,
    ) -> Result<(serde_json::Value, Option<serde_json::Value>, Vec<serde_json::Value>), AppError> {
        let file = fs::File::open(installer_path)?;
        let mut archive = zip::ZipArchive::new(file)?;

        let mut version_json = self.extract_version_json(&mut archive, neoforge_version_name)?;
        self.update_version_json(&mut version_json, game_version, neoforge_version_name);
    ...
    fn extract_version_json(
        ...
        let mut f = archive.by_name("version.json")
            .map_err(|_| AppError::InvalidParam(format!(
                "No version.json in NeoForge installer for {}",
    ...
    fn update_version_json(
        ...
        if let Some(obj) = version_json.as_object_mut() {
            obj.insert("id".to_string(), serde_json::json!(neoforge_version_name));
    ...
    fn run_neoforge_processors(
        ...
        let data_dir = self.limen_dir.join("minecraft").join("neoforge_data_temp");
        ...
        for proc_entry in processors {
            if !self.should_run_processor(proc_entry) {
                eprintln!("Skipping processor (OS mismatch)");
                continue;
            }

            let jar_name = proc_entry.get("jar").and_then(|j| j.as_str()).unwrap_or("");
            eprintln!("Checking processor: {}", jar_name);

            if self.check_outputs(proc_entry, &data_map, &libraries_dir, &minecraft_jar, &data_dir, installer_path, &minecraft_dir, game_version) {
                eprintln!("Processor outputs already exist, skipping");
                continue;
            }
            ...
            if !jar_path.exists() {
                eprintln!("NeoForge processor JAR not found: {:?}", jar_path);
                continue;
            }

            eprintln!("Running processor: {}", jar_name);
            if let Err(e) = self.run_processor(&java_cmd, &jar_path, proc_entry, &data_map, &libraries_dir, &minecraft_jar, &data_dir, installer_path, &minecraft_dir, game_version) {
                eprintln!("NeoForge processor failed: {:?}", e);
                continue;
            }
            eprintln!("Processor completed successfully");
        }
    ...
    fn run_processor(
        ...
        let args = self.build_processor_args(proc_entry, data_map, libraries_dir, minecraft_jar, data_dir, installer_path, minecraft_dir, game_version)?;
        ...
        if !output.status.success() {
            ...
            eprintln!("Processor {} failed with exit code: {:?}", jar_name, output.status.code());
            eprintln!("STDOUT: {}", String::from_utf8_lossy(&output.stdout));
            eprintln!("STDERR: {}", String::from_utf8_lossy(&output.stderr));
            return Err(AppError::ApiError(format!("NeoForge processor {} failed", jar_name)));
        }
        
        // Log successful processor output
        if !output.stdout.is_empty() {
            eprintln!("Processor STDOUT: {}", String::from_utf8_lossy(&output.stdout));
        }
        if !output.stderr.is_empty() {
            eprintln!("Processor STDERR: {}", String::from_utf8_lossy(&output.stderr));
        }
        ...
        if let Some(proc_args) = proc_entry.get("args").and_then(|a| a.as_array()) {
            for a in proc_args {
                if let Some(s) = a.as_str() {
                    args.push(self.resolve_arg(s, data_map, libraries_dir, minecraft_jar, data_dir, installer_path, minecraft_dir, game_version)?);
                }
            }
        }
        Ok(args)
    ...
    fn resolve_arg(
        ...
        if value.starts_with('{') && value.ends_with('}') {
            ...
            return match var {
                ...
                "MINECRAFT_VERSION" => Ok(game_version.to_string()),
                _ => {
    ...
    fn extract_from_jar(&self, jar_path: &Path, inner_path: &str) -> Result<String, AppError> {
        let temp_dir = self.limen_dir.join("minecraft").join("neoforge_data_temp").join("extracted");
    ...
    fn check_outputs(
        ...
        for (key, _) in outputs {
            match self.resolve_arg(key, data_map, libraries_dir, minecraft_jar, data_dir, installer_path, minecraft_dir, game_version) {
                Ok(path) => {
                    let exists = Path::new(&path).exists();
                    if !exists {
                        eprintln!("NeoForge processor output missing: {}", path);
                        all_exist = false;
                    } else {
                        eprintln!("NeoForge processor output exists: {}", path);
                    }
                }
                Err(e) => {
                    eprintln!("Failed to resolve processor output: {:?}", e);
                    return false;
                }
            }
        }
    ...
    // Download libraries from NeoForge Maven and fallback repositories
